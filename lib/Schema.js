...
class Schema {
  ...
  commitHandler = (shareRequest, done) => {
    ...
    if (opData.create) {
      ...
      try {
        var contexts = this.getContexts(rootSchema, doc, paths);
      } catch (err) {
      ...
      var validateCreate = (err) => {
        ...
        done(formatError(err || this.validate(doc, rootSchema, paths, contexts)));
      };
      ...
      opData.validate = (opData, data) => {
        ...
        return this.validate(doc, rootSchema, paths, op.contexts);
      };
      ...
      for (var i = 0; i < opData.op.length; i++) {
        ...
        try {
          var schema = this._getSchemaForPaths(rootSchema, paths);
          var contexts = this.getContexts(schema, value, paths);
        } catch (err) {
          return done(formatError(this._getError(err)));
        }
        ...
        this.runAsyncs(contexts, counter, done);
      }
      if (counter.count === 0 && !counter.sent) {
        counter.sent = true;
        ...
        for (var key in value) {
          try {
            results = results.concat(
              this.getContexts(
                this._getSchemaForObjectProperty(schema, key),
                value[key],
                defaultPaths,
                paths.concat([key]),
              ),
  ...
  _getSchemaForObjectProperty = (schema, property) => {
